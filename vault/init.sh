...
# Configure cluster A
kubectl exec -ti vault-0 -- vault operator init -key-shares=1 -key-threshold=1 -format=json > cluster-a-keys.json
sleep 15
...
# Configure cluster B
kubectl exec -ti vault-3 -- vault operator init -key-shares=1 -key-threshold=1 -format=json > cluster-b-keys.json
sleep 15
...
kubectl exec -ti vault-3 -- vault operator unseal $VAULT_UNSEAL_KEY_CLUSTER_B
